...
def fill_depth_in_mask(depth_image, mask_polygon, inpaint_radius=5):
    ...
    cv2.fillPoly(bin_mask, [np.asarray(mask_polygon, dtype=np.int32)], 255)
    to_inpaint = ((depth == 0) & (bin_mask > 0)).astype(np.uint8)
    if to_inpaint.sum() == 0:
        return depth
    filled = cv2.inpaint(depth.astype(np.float32), to_inpaint, inpaint_radius, cv2.INPAINT_NS)
...
def mask_to_3d_centroid(depth_image, camera_info, mask_polygon, fill_missing_depth=True):
    ...
    bin_mask = np.zeros((h, w), dtype=np.uint8)
    cv2.fillPoly(bin_mask, [np.asarray(mask_polygon, dtype=np.int32)], 1)
    d_use = fill_depth_in_mask(depth_image, mask_polygon) if fill_missing_depth else np.asarray(depth_image, dtype=np.uint16)
    ys, xs = np.where((bin_mask > 0) & (d_use > 0))
    if len(xs) == 0:
        return None, d_use
    z_mm = d_use[ys, xs].astype(np.float64)
    z_m = z_mm / 1000.0
